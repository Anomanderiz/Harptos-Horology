...
def pip_for_day(m: int, d: int):
    ...
    dots = []
    if has_new:
        dots.append(ui.span(class_="pip pip-new", title="New Moon"))
    if has_full:
        dots.append(ui.span(class_="pip pip-full", title="Full Moon"))
...
def month_card(m: int, cur: Optional[HarptosDate]) -> ui.TagChild:
    ...
    for r in range(3):
        start = r * 10 + 1
        rows.append(
            ui.div(*[day_cell(view_year, m, d, is_hl(d)) for d in range(start, start + 10)], class_="day-row")
        )
