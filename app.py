# app.py
# ------------------------------------------------------------------------------
# Harptos – Year at a glance + Month Zoom
# - Year view: 12 months, no inner scrollbars, responsive
# - Month view: full-screen month with large tiles like the reference image
# - Events shown in cells; click chip to edit; click day for list + "Add New"
...
DAYS_PER_MONTH = 30
FESTIVALS: Dict[int, str] = {1:"Midwinter", 4:"Greengrass", 7:"Midsummer", 9:"Highharvestide", 11:"Feast of the Moon"}
...
def event_chips(y: int, m: int, d: int, *, zoom: bool=False):
    ...
    overflow = len(day_events) - max_items
    if overflow > 0:
...
def day_cell(y: int, m: int, d: int, hl: bool, *, zoom: bool=False):
    ...
    return ui.div(ui.input_action_button(pid, str(d), class_=" ".join(classes)),
                  pip_for_day(m,d), event_chips(y,m,d, zoom=zoom), class_=("day-cell zoom" if zoom else "day-cell"))
...
def festival_cell(y:int,m:int, *, zoom: bool=False):
    ...
    return ui.div(ui.div(ui.input_action_button(pid,"31",class_="btn btn-outline-light day-btn"),
                  ...
                  event_chips(y,m,31, zoom=zoom), class_=("day-cell festival-cell zoom" if zoom else "day-cell festival-cell"))
...
def month_zoom_card(m:int, cur:Optional[HarptosDate]):
    """Full-screen month with big tiles (3×10), like the provided example."""
    y=(cur or {"year":1492})["year"]; hl=lambda d: bool(cur and cur["month"]==m and cur["day"]==d)
    rows=[]
    for r in range(3):
        start=r*10+1
